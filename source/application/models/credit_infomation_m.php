...
class credit_infomation_m extends CI_Model
{

    var $CLASS_NAME = 'credit_infomation_m';                   // Class name
    var $TABLE_NAME = 'credit_infomation';                     // Table name
     ...
     * @param   var      $crdt_code                                : 検索キー（ クレジット種別コード）
     * @return  array   $result                                  : 戻り値
     *                      ['err_code']                         : エラーコード(true=正常終了/ false=異常終了)
     *                      ['mssg']                             : エラーメッセージ
     *                      ['row']                              : 検索で取得したクレジット情報テーブルのレコード
     *
     */
    function select($htl_infrmtn_unq_id, $crdt_code)
    {
        //-------
        // 初期値
        //-------
        ...
        $this->db->where('crdt_code',$crdt_code);
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $row)
            {
                ...
                $rec['crdt_code']                            = $row->crdt_code;
                $rec['img_url']                            = $row->img_url;
                $rec['crdt_name']                            = $row->crdt_name;
            }
        }
        else
        {    // 該当データなし
    ...
    /*
     * クレジット情報テーブルのレコードリストを取得する
     *
     * @param   array   $params : パラメータ
     * @return  array   $result :  戻り値
     *              ['err_code']: エラーコード(true=正常終了/ false=異常終了)
     *              ['mssg']    : エラーメッセージ
     *              ['list']    : 検索で取得したクレジット情報テーブルのリスト
     *
     */
