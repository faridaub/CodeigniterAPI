...
class keyword_information_m extends CI_Model
{

    var $CLASS_NAME = 'keyword_information_m';                   // Class name
    var $TABLE_NAME = 'keyword_information';                     // Table name
    ...
    function select($unq_id)
    {
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $row)
            {
                ...
                $rec['kywrd_type']                            = $row->kywrd_type;
                $rec['kywrd_name']                            = $row->kywrd_name;
                $rec['rt_name']                            = $row->rt_name;
                $rec['mns_trnsprttn']                            = $row->mns_trnsprttn;
                $rec['time_rqrd']                            = $row->time_rqrd;
            }
        }
        else
        {    // 該当データなし
    ...
    /*
     * キーワード情報テーブルのレコードリストを取得する
     *
     * @param   array   $params : パラメータ
     * @return  array   $result :  戻り値
     *              ['err_code']: エラーコード(true=正常終了/ false=異常終了)
     *              ['mssg']    : エラーメッセージ
     *              ['list']    : 検索で取得したキーワード情報テーブルのリスト
     *
     */
