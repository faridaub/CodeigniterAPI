...
class Search_booking_api extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->FNCTIN = Api_const::A014;	//予約情報検索API
	}
	public function index()
	{
		...
		try {
			...
			$method = "com.toyokoinn.api.service.SmartphoneApplicationReservationService.searchReservationInformation";
			...
			if (!$this->xmlrpc->send_request()) {
				...
				if ($b_result['errrCode'] == Api_const::BCMN0000){
					...
					$jsonOut = array(
	 					'nmbrRsrvtns' => $b_result['nmbrRsrvtns'],
	 					'rsrvtnInfrmtn' => $b_result['rsrvtnInfrmtn']
					);
					for($i=0; $i<count($b_result['rsrvtnInfrmtn']);$i++){
						$recHotel = $this->hotel_info_m->select($rqst['applctnVrsnNmbr'], $b_result['rsrvtnInfrmtn'][$i]['htlCode'], $rqst['lngg']);
						if ($recHotel['errCode']==true){
							$pymntPrcIncldngTax=$jsonOut['rsrvtnInfrmtn'][$i]['pymntPrcIncldngTax'];
							$pymntPrc=$jsonOut['rsrvtnInfrmtn'][$i]['pymntPrc'];
							$jsonOut['rsrvtnInfrmtn'][$i]['pymntPrcIncldngTax']=$this->api_util->priceFormat($pymntPrcIncldngTax, $recHotel['rec']['crrncy_name'], $recHotel['rec']['crrncy_sign'], $rqst['lngg'],Api_const::RATE_DSPLY_FLG_ON);
							$jsonOut['rsrvtnInfrmtn'][$i]['pymntPrc']=$this->api_util->priceFormat($pymntPrc, $recHotel['rec']['crrncy_name'], $recHotel['rec']['crrncy_sign'], $rqst['lngg']);
							$jsonOut['rsrvtnInfrmtn'][$i]['roomName']=$jsonOut['rsrvtnInfrmtn'][$i]['roomTypeName'];
							$jsonOut['rsrvtnInfrmtn'][$i]['imgURL']=$recHotel['rec']['img_url'];
						}
					}
				}
