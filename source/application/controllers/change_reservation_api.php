...
class Change_reservation_api extends CI_Controller {
	var $FNCTIN = null;
	function __construct()
	{
		...
		$this->FNCTIN = Api_const::A017;	//予約変更API
	...
	public function index()
	{
		...
		try {
			...
			for ($nmbr=1; $nmbr<=$rqst['nmbrRms']; $nmbr++ ) {
				...
				$param['htlCode'] = $rqst[$rmNmbr.'_htlCode'];           // ホテルコード
...
//					$sbttlPrc=$this->api_util->getAmount($sbttPrcIncldngTax, $oldRow['cnsmptn_tax_rate']);
				...
				array(
						...
						'htlCode'=>		 		new XML_RPC_Value($rqst[$rmNmbr.'_htlCode'], 'string')
				);
			...
			array(
					...
					new XML_RPC_Value($this->config->config['prcssng_type_change'],'string'),
					new XML_RPC_Value($rqst[$rmNmbr.'_rsrvId'],'string'),
