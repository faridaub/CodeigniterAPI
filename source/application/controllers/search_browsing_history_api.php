...
class Search_browsing_history_api extends CI_Controller {
	...
	public function index()
	{
		...
		try {
			...
			log_d("chkApiCommon Start");
			// API共通チェック
			$chk_cmmn = $this->api_util->chkApiCommon($rqst, $ids);
			log_d("chkApiCommon End");
			if ($chk_cmmn['errCode'] !== true) {
 			...
 			$method 	= "com.toyokoinn.api.service.SmartphoneApplicationCustomerService.searchBrowsingHistory";
 			...
 			$wkFavorite = array(
 					'nmbrBrwsngHstry' => $b_result['nmbrBrwsngHstry'],
 					'brwsngHstryList' => $b_result['brwsngHstryList']
 			);
 			...
 			foreach($wkFavorite['brwsngHstryList'] as $row ){
...
// 			$jsonOut['nmbrBrwsngHstry'] = count($myFvrtsInfrmtnList);
 			$jsonOut['nmbrBrwsngHstry'] = $b_result['nmbrBrwsngHstry'];
 			$jsonOut['brwsngHstryInfrmtn'] = $myFvrtsInfrmtnList;
		} catch (Exception $e) {
